...
|-----------|-------------|--------|
| **JavaScript** | `@managersaas/js-sdk` | ✅ Disponível |
| **Python** | `managersaas-python` | ✅ Disponível |
| **PHP** | `managersaas-php` | 🚧 Em desenvolvimento |
| **Java** | `managersaas-java` | 📋 Planejado |

### **🔧 Exemplo de Uso com SDK**

```javascript
import { ManagerSaaS } from '@managersaas/js-sdk';

const client = new ManagerSaaS({
  apiKey: 'sua_api_key',
  environment: 'production'
});

const cte = await client.cte.create({
  // dados do CT-e
});
```
